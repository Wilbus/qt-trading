...
public:
    explicit QCPPaintBufferGlFbo(const QSize& size,
        ...
        QWeakPointer<QOpenGLContext>
            glContext,
        QWeakPointer<QOpenGLPaintDevice>
            glPaintDevice);
    ...
    foreach (QCPAbstractItem* item, mItems)
    {
        ...
        if (!currentItem ||
            (onlySelectable && !currentItem->selectable())) // we could have also passed onlySelectable to the
                                                            // selectTest function, but checking here is faster, because
                                                            // we have access to QCPAbstractItem::selectable
            continue;
